...
const BuilderRadioLabel = styled('label')`
  position: relative;
  display: block;
  width: 9.2rem;
  min-width: 9.2rem;
...
const BuilderRadioInput = styled('input')`
  position: absolute;
  border: 0;
  clip: rect(0 0 0 0);
  clip-path: inset(50%);
...
const BuilderRadioButton = styled('span')`
  content: '';
...
const BuilderRadio = ({ option, handler }) => {
  return (
    <BuilderRadioLabel htmlFor={option.id}>
      <BuilderRadioInput
        id={option.id}
        type="radio"
